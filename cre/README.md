# SyndicateLend confidential interest accrual (Chainlink CRE)

Workflow `interest-accrual` is a **Confidential Workflow**: the handler runs inside a TEE (AWS Nitro, us-west-2) so the administrative agent's rate notice is never visible to node operators.

What happens on each run (cron, daily 09:00 UTC):

1. Read the latest **notice commitment** from the public Hedera Consensus Service topic (mirror node). The commitment is `keccak256(abi.encode(facilityId, periodId, periodStart, periodEnd, rateBps, dayCountBasis, nonce))` with a random 32-byte nonce.
2. Inside the enclave, fetch the **private notice** from the agent endpoint (`/api/agent/notice`) with the bearer token released by the Vault DON (`API_TOKEN`).
3. Recompute the commitment. If the notice was altered, the run aborts (`tamper-settings` target proves this).
4. Read each register holder's par balance from the ATS security via the mirror node contract-call API.
5. Compute interest per holder in integers (par x rate x days / basis, 6 dp mock USD) and cross back to the DON with only the distribution: `(bytes32 commitment, uint256 periodId, address[] holders, uint256[] amounts)` as a signed report. The rate, basis and nonce stay inside the enclave.

```bash
# prerequisites: the agent service running (web/: npm run dev) and a commitment published
cd web && npx tsx scripts/publish-notice.mts 725 30      # 7.25% all-in, 30-day period
cd ../cre
cre workflow simulate interest-accrual --target staging-settings --non-interactive --trigger-index 0
cre workflow simulate interest-accrual --target tamper-settings  --non-interactive --trigger-index 0   # must abort
```

Secrets: `secrets.yaml` maps `API_TOKEN` to `SECRET_API_TOKEN` in `cre/.env` (gitignored); the same value is `NOTICE_API_TOKEN` in the repo-root `.env` for the endpoint. Deployment of Confidential Workflows requires private-beta enrollment; simulation does not.
