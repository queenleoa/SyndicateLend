...
| 2026-09-14 | Scaling the feeder demo to 300 holders stopped twice: the feeder account that signs each par transfer ran out of HBAR (each transfer reserves about 3.6 HBAR of gas), and on resume a half-onboarded holder failed with `TOKEN_ALREADY_ASSOCIATED_TO_ACCOUNT`. | Feeder run log; holders 71 and onward. | The feeder script is resumable at every step (existing account, association, control list, KYC and balance are checked before being repeated); the feeder account was topped up. 300 holders, 1,800 transactions; period 4 computed for 310 holders in one enclave call and paid to 308 in 35 batches. |
