...
| Key quorum per institution: the three users, threshold 2 | provision | any two of three authorise |
...
| Desk actions as intents (`eth_signTransaction`) on the desk wallet; approvals via each member's own session (user signing key) | `web/src/lib/approvals.ts` | 72-hour expiry by default |
...
2. **Access control** (Users > Access control): enable the **allowlist** and add the desk members' emails (or your company domain). Unknown emails then cannot even create an account. The app does not enforce this itself; it is the first gate.
...
- Authorization keys held by the server as quorum members: the quorum is made of Privy users, so approvals are tied to people and their MFA, and the app secret alone can never move funds.
