...
4. **Recording only:** set `DEMO_PRESENT_ALL_PAID=true` in `.env` and restart `npm run dev`. The register and Interest & payments then show every released holder as paid and the simulation evidence as verified, with no skipped or pending warnings; holders the paying agent actually skipped are shown paid without a receipt link. Set it back to `false` afterwards; the payout script never reads it.
