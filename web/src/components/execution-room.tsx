...
type Register = { facility: { name: string; symbol: string; tokenId: string; evmAddress: string; units: string; maturity: number }; engine: { address: string }; mockUsd: { tokenId: string }; topics: { notices: string }; holders: { id: string; name: string; eligible: boolean; balances: { par: string; usd: string } | null }[] };
...
export function ExecutionRoom() {
  ...
  const stage = latestTrade?.state === "Settled" ? 4 : latestTrade?.state === "Scheduled" || latestTrade?.state === "Failed" ? 3 : latestTrade ? 2 : latestRfq?.quotes.length ? 1 : 0;
  ...
  return (
    <div className="execution-room-page">
      ...
      <section className="room-lower">
        <div className="card-flat room-panel"><div className="room-panel-head"><div><span>ELIGIBLE REGISTER</span><h3>Institutional holders</h3></div><Link href="/register">Full register →</Link></div><div className="holder-list">{register?.holders.map((h) => <div key={h.id}><span className="holder-avatar">{h.name.slice(0, 1)}</span><div><strong>{h.name}</strong><small>{h.eligible ? "KYC + whitelist active" : "Transfer restricted"}</small></div><b>{h.balances ? `$${par(h.balances.par)}` : "—"}<small>PAR</small></b></div>)}</div></div>
