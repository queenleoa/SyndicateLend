...
for (const it of (await listWalletIntents(inst.wallet!.id)) as any[]) {
  const q = it.authorization_details[0];
  const signed = q?.members.filter((m: any) => m.signed_at).map((m: any) => `${m.user_id}@${new Date(m.signed_at).toISOString()}`);
